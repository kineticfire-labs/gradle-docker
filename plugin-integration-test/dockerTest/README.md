# dockerTest Integration Tests

Integration tests for the `dockerTest` task (Docker Compose orchestration).

## Purpose

These integration tests validate the `dockerTest` plugin functionality by:
...
The `dockerTest/` tests are organized into two distinct categories:
...
1. **Recommended Pattern (All Examples):**
   - Define stack in `dockerTest.composeStacks { }` in `build.gradle`
...
- Each stack defined in `dockerTest { composeStacks { ... } }` generates these tasks
...
## Running dockerTest Tests

**⚠️ All commands must be run from `/plugin-integration-test/` directory.**

```bash
# Run all dockerTest integration tests (verification + examples)
...
./gradlew -Pplugin_version=<version> dockerTest:verification:wait-healthy:integrationTest

# Run specific example test
./gradlew -Pplugin_version=<version> dockerTest:examples:web-app:integrationTest
./gradlew -Pplugin_version=<version> dockerTest:examples:database-app:integrationTest
...
- Configured via `dockerTest { composeStacks { stackName { projectName = "unique-name" } } }`
