...
These demonstrate how to use the dockerTest plugin for testing your applications with both **Spock** and **JUnit 5**
...
1. **Configure stack in `build.gradle`** → Define your Docker Compose stack in the `dockerTest.composeStacks { }` DSL
...
Example tests show **real-world usage** of the dockerTest plugin:
...
- Complete dockerTest configuration
...
```bash
./gradlew dockerTest:examples:integrationTest
```
...
```bash
# Spock + CLASS
./gradlew :dockerTest:examples:web-app:integrationTest           # Basic REST API
./gradlew :dockerTest:examples:database-app:integrationTest      # Multi-service + PostgreSQL
./gradlew :dockerTest:examples:stateful-web-app:integrationTest  # Stateful workflows

# JUnit 5 + CLASS
./gradlew :dockerTest:examples:web-app-junit:integrationTest     # Basic REST API
```

**METHOD Lifecycle Examples** (complete isolation, containers restart per test):
```bash
# Spock + METHOD
./gradlew :dockerTest:examples:isolated-tests:integrationTest    # Proves isolation

# JUnit 5 + METHOD
./gradlew :dockerTest:examples:isolated-tests-junit:integrationTest  # Proves isolation
```
...
- **[dockerTest DSL Usage Guide](../../docs/usage/usage-docker-orch.md)** - Complete guide to Docker Compose
  orchestration
...
- [ ] **Test execution**:
  - [ ] Run: `./gradlew :dockerTest:examples:<example-name>:integrationTest`
