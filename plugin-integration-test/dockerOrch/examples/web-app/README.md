...
- ✅ How to configure Docker Compose orchestration
- ✅ How to wait for containers to be healthy
- ✅ How to read port mappings from state files
- ✅ How to test REST endpoints with RestAssured
- ✅ How to test concurrent request handling
...
./gradlew -Pplugin_version=1.0.0-SNAPSHOT \
  dockerOrch:examples:web-app:app-image:runIntegrationTest
```

This runs the complete workflow:
1. Build app JAR (`bootJar`)
2. Build Docker image (`dockerBuildWebApp`)
3. Start Docker Compose (`composeUpWebAppTest`)
4. Wait for container to be HEALTHY
5. Run integration tests (`integrationTest`)
6. Stop Docker Compose (`composeDownWebAppTest`)
...
./gradlew -Pplugin_version=1.0.0-SNAPSHOT \
  dockerOrch:examples:web-app:clean

# Build the Spring Boot app
./gradlew -Pplugin_version=1.0.0-SNAPSHOT \
  dockerOrch:examples:web-app:app:bootJar

# Build Docker image
./gradlew -Pplugin_version=1.0.0-SNAPSHOT \
  dockerOrch:examples:web-app:app-image:dockerBuildWebApp

# Start Docker Compose stack
./gradlew -Pplugin_version=1.0.0-SNAPSHOT \
  dockerOrch:examples:web-app:app-image:composeUpWebAppTest

# Run tests (requires stack to be up)
./gradlew -Pplugin_version=1.0.0-SNAPSHOT \
  dockerOrch:examples:web-app:app-image:integrationTest

# Stop Docker Compose stack
./gradlew -Pplugin_version=1.0.0-SNAPSHOT \
  dockerOrch:examples:web-app:app-image:composeDownWebAppTest
```
...
./gradlew -Pplugin_version=1.0.0-SNAPSHOT \
  dockerOrch:examples:web-app:clean \
  dockerOrch:examples:web-app:app:clean \
  dockerOrch:examples:web-app:app-image:clean
```
...
- Verify state file exists: `build/compose-state/webAppTest-state.json`
