...
/**
 * Unit tests for DockerImageCleanTask.
 * 
 * Tests task behavior without actually calling Docker commands.
 * Focuses on input validation and task configuration.
 */
class DockerImageCleanTaskTest extends Specification {

    Project project
    DockerImageCleanTask task

    def setup() {
        project = ProjectBuilder.builder().build()
        task = project.tasks.register('testCleanDockerImages', DockerImageCleanTask).get()
    }

    def "task has correct group and inputs"() {
        when:
        task.imageNames.set(['test-image:1.0'])
        
        then:
        task.group == null // Group set by convention plugin, not task itself
