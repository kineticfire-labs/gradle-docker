...
class DockerComposeMethodExtensionWaitForLogTest extends Specification {
    ...
    def "parseIntProperty handles whitespace value '#value' by returning default"() {
        ...
        expect:
        // Current implementation treats pure whitespace like empty string
    ...
    def "parseBooleanProperty parses '#value' as #expected (Boolean.parseBoolean semantics)"() {
        ...
        expect:
        // Boolean.parseBoolean returns true only for "true" (case-insensitive)
    ...
    def "performWaitForLog handles empty parsed services map"() {
        given:
        // Invalid JSON that parses to empty map
    ...
    private int invokeParseIntProperty(String propertyName, int defaultValue) {
        Method method = DockerComposeMethodExtension.getDeclaredMethod(
