...
**Package Coverage (as of 2025-12-22):**

| Package | Instructions | Branches |
|---------|--------------|----------|
| `com.kineticfire.gradle.docker.junit` | 92.2% | 80.5% |
| `com.kineticfire.gradle.docker.junit.service` | 96.7% | 95.2% |
...
##### 3e. Groovy Closure Bytecode (`_parsePortMappings_closure6`)

**What:**
- Groovy-generated bytecode for closure handling in `parsePortMappings()` method
- 66% instruction coverage (41 of 121 instructions missed), 100% branch coverage

**Why Unit Testing is Impractical:**
- Groovy generates additional bytecode for closure handling including:
  - Closure initialization and delegate resolution
  - Dynamic type coercion and casting
  - GString interpolation machinery
  - Null safety checks
- These are internal Groovy runtime instructions, not application logic
- All logical branches (6 of 6) are covered

**Mitigations:**
- All testable logic paths are covered
- Branch coverage is 100%
- Missing instructions are language runtime, not business logic

##### 3f. Unreachable Branches Due to LogsConfig Normalization

**What:**
- `captureLogs()` line 228: `if (config.tailLines > 0)` - 1 of 2 branches missed
- `captureLogs()` line 232: `if (config.services && !config.services.isEmpty())` - 2 of 8 branches missed

**Why Unit Testing is Impractical:**
- `LogsConfig` constructor normalizes inputs:
  - `tailLines` uses `Math.max(1, tailLines)`, ensuring `tailLines >= 1`
  - `services` uses `services ?: []`, ensuring `services` is never null
- The "else" branches are unreachable due to this normalization
- Short-circuit evaluation in boolean conditions creates additional unreachable branches

**Mitigations:**
- Defensive code at configuration layer
- All reachable paths are tested
- Normalization ensures safe defaults

##### 3g. Short-Circuit Tautology in parseServiceState

**What:**
- Line 310: `lowerStatus.contains('restart') || lowerStatus.contains('restarting')` - 1 of 6 branches missed

**Why Unit Testing is Impractical:**
- The string 'restarting' contains 'restart'
- If status contains 'restarting', the first condition (`contains('restart')`) is true
- Short-circuit evaluation skips the second condition
- A case where `contains('restart')` is false but `contains('restarting')` is true is impossible

**Mitigations:**
- Logical tautology creates unreachable branch
- Both "restart" and "restarting" statuses are tested and handled correctly

##### 3h. For Loop Compiler-Generated Branches

**What:**
- Lines 177, 267: `for` loops have 1 of 4 branches missed each

**Why Unit Testing is Impractical:**
- Groovy/Java for-each loops generate 4 bytecode branches:
  1. Iterator null check (entry)
  2. hasNext() true (continue iteration)
  3. hasNext() false (exit loop)
  4. Collection null/empty handling
- The null iterator branch is unreachable when collections are guaranteed non-null

**Mitigations:**
- Collections are validated before iteration
- Standard compiler-generated branches
- All testable iteration scenarios are covered
...
- **2025-12-22**: Improved `com.kineticfire.gradle.docker.junit.service` package coverage from 96.2%/94.4%
  ...
  InterruptedException handling, and single service checks. Documented remaining gaps: Groovy closure
  bytecode (41 instructions), LogsConfig normalization (3 branches), short-circuit tautology (1 branch),
  and for-loop compiler-generated branches (2 branches). Added DockerProjectTranslator closure gap
  documentation. Updated summary statistics to reflect ~2290 lines of documented external boundaries.
