...
The `dockerTest` DSL provides two ways to orchestrate Docker Compose in your integration tests:
...
- **`dockerTest.composeStacks`** defines **WHAT** the stack is (files, health checks, project name)
...
- **`docker.compose.stack`** - Stack name from dockerTest DSL
...
- Check that stack name in system property matches dockerTest DSL
...
- Check test task has system properties configured:
  ...
  ```
- Verify dockerTest DSL has matching stack configured
...
- `plugin-integration-test/dockerTest/examples/web-app/` - Spock-based tests
- `plugin-integration-test/dockerTest/examples/web-app-junit/` - JUnit-based tests
- `plugin-integration-test/dockerTest/examples/isolated-tests/` - Test isolation pattern
- `plugin-integration-test/dockerTest/verification/basic/` - Minimal setup
